...
void OledClear(void);
# 12 "src/user.c" 2
# 1 "d:\\downloads\\soft\\mplab ide\\xc32 compiler\\pic32mx\\include\\sys\\attribs.h" 1 3
# 48 "d:\\downloads\\soft\\mplab ide\\xc32 compiler\\pic32mx\\include\\sys\\attribs.h" 3
...
extern int convertWiFIREadc(__uint8_t channelNumber) ;
# 15 "src/user.c" 2
enum eDirection dir;
...
void oled_init(){
    ...
    OledSetDrawColor(0);
    OledSetDrawMode(0);
    OledSetFillPattern(OledGetStdPattern(1));
...
void input(){
    if((convertWiFIREadc(10) > 3276) && dir!= RIGHT){
        dir = LEFT;
    }
    else if((convertWiFIREadc(10) < 819)&& dir!= LEFT ){
        dir = RIGHT;
    }
    else if((convertWiFIREadc(13) > 3276)&& dir!= DOWN){
        dir = UP;
    }
    else if((convertWiFIREadc(13) < 819)&& dir!= UP){
        dir = DOWN;
    }
...
void logic(){
    __int32_t prevX = tailX[0];
    __int32_t prevY = tailY[0];
    __int32_t prev2X,prev2Y;
    __int32_t i,j;
...
void set_leds(__int32_t leds)
{
   LATGbits.LATG6 = (leds & (1 << 0)) ? 1 : 0;
   LATDbits.LATD4 = (leds & (1 << 1)) ? 1 : 0;
   LATBbits.LATB11 = (leds & (1 << 2)) ? 1 : 0;
   LATGbits.LATG15 = (leds & (1 << 3)) ? 1 : 0;
}
...
void led_test(void){
 ...
 void __attribute__((vector(16), interrupt(IPL2SRS), nomips16)) Capture3(void){
